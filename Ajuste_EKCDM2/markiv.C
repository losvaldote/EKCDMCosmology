...
void markiv() {
	...
	Int_t counter = 0;
	Int_t bins = 500;
	Float_t minHist = -0.02;
	Float_t maxHist = 0.025;

	//Para ver cuántos datos se tiene

	ifstream datos("wMO.txt");
	string line;
	while(getline(datos,line)){
		Float_t event = 0;
		event = atof(line.c_str());
		if (event <= 0.025 && event >= -0.02)
		counter ++;
	}	
	...
	TH1F *h = new TH1F("h","wMO",bins,minHist,maxHist); //( , , #bin,Inf,Max)

	//Llenar el histograma
	ifstream datos2("wMO.txt");
	//string line;
